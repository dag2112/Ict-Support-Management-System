...
export default function AssignPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.getRequests({ status: "APPROVED" }),
      api.getTechnicians(),
    ]).then(([reqRes, techs]) => {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">Assign Technician</h1>
      {loading && <p className="text-gray-400">Loading...</p>}
      <div className="space-y-4">
        {!loading && requests.length === 0 && <p className="text-gray-400">No approved requests awaiting assignment.</p>}
        {requests.map((r) => (
          <div key={r.id} className="bg-white rounded-xl shadow p-5 flex items-center justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">{r.submittedBy?.name} · {r.issueType} · <span className={r.urgency === "HIGH" ? "text-red-600" : r.urgency === "MEDIUM" ? "text-yellow-600" : "text-green-600"}>{r.urgency}</span></p>
            </div>
