  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        ...
        {spares.map((s) => (
          <div key={s.id} className="bg-white dark:bg-gray-800 rounded-xl shadow p-5 flex items-center justify-between gap-4 border border-gray-100 dark:border-gray-700">
            <div>
              <span className="font-mono text-blue-700 dark:text-blue-400 text-xs">{s.request?.requestNumber}</span>
              <p className="font-semibold text-gray-800 dark:text-white mt-1">{s.spareName}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">Qty: {s.quantity} · By: {s.requestedBy?.name}</p>
            </div>
            <div className="flex items-center gap-3">
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusCls(s.status)}`}>{s.status.replace("_", " ")}</span>
              {s.status === "APPROVED" && (
                <>
                  <button onClick={() => allocate(s.id)} disabled={acting === s.id}
                    className="text-sm bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300 px-4 py-1.5 rounded-full hover:bg-teal-200 disabled:opacity-50">
                    {acting === s.id ? "..." : "Allocate"}
                  </button>
                  <button onClick={() => requestPurchase(s.id)} disabled={acting === s.id}
                    className="text-sm bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-4 py-1.5 rounded-full hover:bg-gray-200 disabled:opacity-50">
                    Request Purchase
                  </button>
                </>
              )}
