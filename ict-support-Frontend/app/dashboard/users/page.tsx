          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {users.map((u) => (
              <tr key={u.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3"><span className={`text-xs font-medium ${u.isActive ? "text-green-600" : "text-red-500"}`}>{u.isActive ? "Active" : "Inactive"}</span></td>
                <td className="px-4 py-3">
                  {u.isActive && <button onClick={() => deleteUser(u.id)} className="text-xs text-red-500 hover:text-red-700">Deactivate</button>}
                </td>
              </tr>
            ))}
          </tbody>
